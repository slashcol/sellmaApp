 ...
 */
  ...
  salaChat(id, name, desc, contact, login, username, identificacion) {
  ...
  futbol_local(id, name, desc, contact, login, namedest, ident) {
...
/*   colores(id, name, desc, contact, login, username, identificacion) {
  ...
  loadSaldo() {
    ...
    return new Promise(resolve => {
      ...
      this.postPvdr.postData(body, 'file_aksi.php').subscribe(data => {
        if(data.result) {
    this.resultado = JSON.stringify(data.result)
        }
        console.log(data);
        resolve(true);
      });
